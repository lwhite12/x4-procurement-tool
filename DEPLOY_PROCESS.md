...
```
git add -A
git commit -m "<describe what shipped>

Fixes #<issue>, Fixes #<issue>"
git push origin main
```

Reference any GitHub issues this batch closes (see the repo's own
`Fixes #N` / `Closes #N` convention). Commit *before* deploying, not
after -- the deployed image should always trace back to a real commit, not
a mix of committed and uncommitted state that's hard to reproduce later.
...
`origin/main`, or if `git status` shows unrelated unstaged changes).
